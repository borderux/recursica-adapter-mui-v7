...
- **Spread `sanitizedProps` first, always, and resolve override intent by name — not by ordering** (see the canonical guide's §3.3) — this bit MUI especially hard: `icon`/`checkedIcon` fully _replace_ MUI's rendered visual rather than layering on top of it (see Radio/Checkbox/Switch), so a bare override placed after the spread with no `withCallerOverride` can silently blank out the entire control if a caller happens to pass that same prop. `classes` has the same failure mode from the other direction — a raw caller value would drop any slot key it doesn't mention (unmatched keys vanish with no warning), so route it through `mergeClassNames` rather than a plain override or plain spread.
- **Delete unsupported props with `omitUnsupportedProps`, not one-off `delete restRecord[...]` calls** (see the canonical guide's §3.2). Declare a local `UNSUPPORTED_PROPS` const per component (and per sub-component, e.g. `MenuItem`/`MenuDivider`), one entry per prop with a rationale comment, and pass it to `omitUnsupportedProps` right after `filterStylingProps`. Several components in this adapter still use ad hoc `delete restRecord["size"]`-style calls — consolidate those into `UNSUPPORTED_PROPS` when you touch the file.
